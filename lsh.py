...
from sympy import isprime # for creating candidate pairs in lsh
...
def k_shingles():
    ...
    # print("K-Shingles = " , sum(len(s) for s in docs_k_shingles))
...
def signature_set(k_shingles):
    ...
    # implement your code here
    uwords = set.union(*k_shingles)
    num_elements = len(uwords)
    word_to_index = {word: i for i, word in enumerate(uwords)}
    for shingle in k_shingles:
        ...
        docs_sig_sets.append(row)


    ##PRINT FUNCTIONS FOR TESTING
    # print(docs_sig_sets)
    # print("Uniqeue words = ", len(uwords))
    # print("Signature set dimensions = ", len(docs_sig_sets), " x ", len(docs_sig_sets[0]))
    # print(k_shingles)
    # print(uwords)
...
def generate_hash_functions(num_perm, N):
    ...
    while not isprime(p):
        p += 1
    # print("The number p is = ", p)
    # print("The number N is = ", N)
    for _ in range(num_perm):
        a = random.randint(1,N-1)
        b = random.randint(1,N-1)
        def hashfunction(x, a=a, b=b):
            return (a * x + b) % p
        
        hash_funcs.append(hashfunction)

    ##PRINT FUNCTIONS FOR TESTING
    # print(len(hash_funcs))
    return hash_funcs
...
def minHash(docs_signature_sets, hash_fn):
    ...
    # implement your code here
    num_docs = len(docs_signature_sets)
    num_hashes = len(hash_fn)
    
    min_hash_signatures = np.full((num_hashes, num_docs), np.inf)

    for doc_idx, doc_signature in enumerate(docs_signature_sets):
        for j in range(len(doc_signature)):
            if doc_signature[j] == 1:
                for i, hash_func in enumerate(hash_fn):
                    if min_hash_signatures[i][doc_idx] > hash_func(j+1):
                        min_hash_signatures[i][doc_idx] = hash_func(j+1)
    ...
    # print("Signature Hash Dimension = ",len(min_hash_signatures)," x ",len(min_hash_signatures[0]))
...
def lsh(m_matrix):
    ...
    for band_idx in range(num_bands):
        ...
        band_end = (band_idx + 1) * rows_per_band if band_idx != num_bands - 1 else num_rows

        for col_idx in range(len(m_matrix[0])):
            band_hash = hash(tuple(m_matrix[band_start:band_end, col_idx]))
            if band_hash in hash_tables[band_idx]:
                hash_tables[band_idx][band_hash].append(col_idx)
            else:
                hash_tables[band_idx][band_hash] = [col_idx]


        for hash_val, doc_ids in hash_tables[band_idx].items():
            if len(doc_ids) > 1:
                for i in range(len(doc_ids)):
                    for j in range(i + 1, len(doc_ids)):
                        if (doc_ids[i], doc_ids[j]) not in candidates:
    ...
    #     total_sim = 0
    #     total_el = len(matrix_a)

    #     for row_a, row_b in zip(matrix_a,matrix_b):
    #         sim_count = np.sum(row_a == row_b)
    #         total_sim += sim_count

    #     sim_rate = total_sim / total_el
    #     return sim_rate
    ...
    # # implement your code here
    # b = parameters_dictionary['b'] # Bands
    # t = parameters_dictionary['t'] # Threshold

    # num_docs = len(m_matrix[0])
    # num_rows = len(m_matrix)
    # band_size = num_rows // b

    # buckets = []

    # # Split the matrix into bands
    # for i in range(0, num_rows, band_size):
    #     band = m_matrix[i: i + band_size]
    #    buckets.append(band)
    ...
    # for band in buckets:
    #     for i, col1 in enumerate(band):
    #         for j, col2 in enumerate(band):
    #             if j > i:  # Avoid redundant comparisons
    #                 jacc = jaccard(set(col1), set(col2))
    #                 if jacc >= t:
    ...
    #             col1 = set(band[row][i] for row in range(len(band)))
    #             col2 = set(band[row][j] for row in range(len(band)))
    #             jacc = jaccard(col1,col2)
    #             if jacc >= t:
    #                 if (i,j) not in candidates:
    #                     candidates.append((i, j))
    # Method only checks buckets for pairs
...
def candidates_similarities(candidate_docs, min_hash_matrix):
    ...
    # implement your code here
    # This method checks the whole cols/documents
    permutations = parameters_dictionary['permutations']
    t = parameters_dictionary['t']
    
    for doc1, doc2 in candidate_docs:
        num = 0
        for row in range(len(min_hash_matrix)):
            if min_hash_matrix[row, doc1] == min_hash_matrix[row, doc2]:
                num += 1
        similarity = num / permutations
        if similarity >= t:
            similarity_dict.append({(doc1, doc2): similarity})
